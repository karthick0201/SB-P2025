...
public class Sb07DatajpaJparepositoryFindermethodsApplication implements CommandLineRunner{
	...
	private void hibernateQueryLanguage() {
		...
		System.out.println(">>---------|3. Hibernate Query - SELECT + WHERE Condition + Using 'Named Parameter' |---------<");
		
		int ram = 8;
		List<Labtop> lList = labtopDao.getLabtopBasedOnRam(ram);
		lList.forEach(l -> System.out.println(l));

		System.out.println(">>---------|4. Hibernate Query - SELECT + WHERE Condition + Using 'Normal Parameter'  |---------<");
		int rom= 1024;
		List<Labtop> romLit = labtopDao.getLabtopBasedOnRom(rom);
		romLit.forEach(l -> System.out.println(l));
		
		System.out.println(">>---------|5. Hibernate Query - Non Select Query  |---------<");
		
		int id = 2;
		
		int affected = labtopDao.updateLabtopRom(2, 512);
		
		Optional<Labtop> optional = labtopDao.findById(id);
		
		if(optional.isPresent()) {
			System.out.println("Id : " + id + " OBject : " + optional.get());
		}
	...
	private void nativeQueryLanguagePratice() {
		System.out.println(">>------| Native Query Language");
		
		System.out.println(">>---------|1. Native Query - Select ALL |---------<");
		List<Labtop> labtopList = (List<Labtop>)labtopDao.getLabtopListUsingHQL();
		labtopList.forEach(l -> System.out.println(l));

		System.out.println(">>---------|2. Native Query - Particular Column |---------<");
		List<Object[]> hqlLabtopList = labtopDao.getLabtopSelectColumnUsingHQLQuery();
		
		hqlLabtopList.forEach(oa -> {
			System.out.println(oa[0] + " : " + oa[1] + " : " + oa[2]);
		});
		System.out.println(">>---------|2. Native Query - Non Select Query |---------<");
		int id = 3;
		int isDeleted = labtopDao.deleteFromLabtop(id);
		
		System.out.println(id + "' Is record deleted : " + isDeleted);
